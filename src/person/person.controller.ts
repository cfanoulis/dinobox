import { Body, Controller, Delete, Get, HttpCode, HttpException, HttpStatus, Param, Patch, Put } from '@nestjs/common';
...
export class PersonController {
	...
	@HttpCode(201)
	@Put('new')
	async newPerson(@Body() createData: CreatePersonData) {
		const { data, error } = await this.personService.newPerson(createData);

		if (error || data.length < 1)
			throw new HttpException(`User couldn't be created. Error: ${error ?? 'no user returned'}`, HttpStatus.INTERNAL_SERVER_ERROR);
		return data[0];
	}

	@HttpCode(202)
	@Patch(':id')
	async editPerson(@Param('id') id: string, @Body() editData: Partial<CreatePersonData>) {
